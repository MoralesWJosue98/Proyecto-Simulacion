...
enum parameter {
    DIRICHLET_VALUE, NEUMANN_VALUE, INITIAL_ESTEEM, TIME_STEP, INITIAL_TIME, FINAL_TIME
};
//Enumeración para identificar las cantidades utilizadas en el problema, que también son proveídas en el archivo de entrada
enum quantity {
    NUM_NODES, NUM_ELEMENTS, NUM_DIRICHLET_BCs, NUM_NEUMANN_BCs
};
...
/*
    ...
    La clase además provee de los procedimientos básicos para la manipulación y
...
private:
    /*
        ...
        en el código):
        - Una estructura para definir un arreglo de 6 números reales para
...
public:
    ...
    */
    void init_geometry() {
        ...
        SDDS<int>::extract(quantities, NUM_DIRICHLET_BCs,
                           &n);    //Se extrae la cantidad de nodos con condición de Dirichlet
        SDDS<FEMNode *>::create(&dirichlet_conditions, n,
                                ARRAY);  //Se inicializa el arreglo de condiciones de Dirichlet

        SDDS<int>::extract(quantities, NUM_NEUMANN_BCs,
                           &n);      //Se extrae la cantidad de nodos con condición de Neumann
    ...
    */
    void
    set_parameters(float Ad, float An, float initial_S, float delta_t,
                   float t_0,
                   float t_f) {
        SDDS<float>::insert(parameters, DIRICHLET_VALUE, Ad);
        SDDS<float>::insert(parameters, NEUMANN_VALUE, An);
        SDDS<float>::insert(parameters, INITIAL_ESTEEM, initial_S);
